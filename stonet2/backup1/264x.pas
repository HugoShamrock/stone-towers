uses crt,dos,ems,lostgfxx,myska,graph;
...
begin
  ...
  if p1<>0 then paint(0,  0,zbuffer[0]);
  if p2<>0 then inbuff(p2,0);
  if p2<>0 then paint(0,100,zbuffer[0]);
  if p3<>0 then inbuff(p3,0);
  if p3<>0 then paint(0,200,zbuffer[0]);
  if p4<>0 then inbuff(p4,0);
  if p4<>0 then paint(0,300,zbuffer[0]);
  ukaz_mysku;
end;
...
{  paint(0,0,zbuffer[0]);
  paint(0,100,zbuffer[0]);
  paint(0,200,zbuffer[0]);
  paint(0,300,zbuffer[0]);}
 { cls(buffseg[0],0);
